...
_ANOMALY_CONTEXT = {
    ...
    AnomalyType.RATE_LIMIT_APPROACH: dedent("""
        The integration is consuming GitHub API rate limit at an accelerating rate.
    ...
    AnomalyType.AUTH_FAILURE: dedent("""
        ...
        This typically indicates: expired GitHub token, revoked token,
...
_DIAGNOSIS_PROMPT = dedent("""
    ...
    {{
        ...
        "affected_component": "<one of: github_api | auth_token | network | rate_limiter | webhook | dns | tls | integration_code>",
...
async def diagnose_anomaly(anomaly_id: int) -> None:
    ...
    try:
        ...
        # Extract JSON — find first { and last } regardless of wrapping
        match = re.search(r'\{.*\}', raw_response, re.DOTALL)
        if not match:
            raise json.JSONDecodeError("No JSON found", raw_response, 0)
        diagnosis = json.loads(match.group())

        # Save back to DB — capture values while session is open
        with get_db() as db:
            anomaly = db.query(Anomaly).filter(Anomaly.id == anomaly_id).first()
            if not anomaly:
                return

            anomaly.root_cause         = diagnosis.get("root_cause", "Unknown")
            anomaly.confidence         = float(diagnosis.get("confidence", 0.5))
            anomaly.affected_component = diagnosis.get("affected_component", "unknown")
            anomaly.fix_steps          = json.dumps(diagnosis.get("fix_steps", []))
        ...
        logger.info(
            "Diagnosis complete for anomaly %d: %s (confidence=%.2f)",
            anomaly_id,
            diagnosis.get("root_cause", "?")[:80],
            diagnosis.get("confidence", 0),
        )

        _append_runbook(anomaly_id, diagnosis, a_type, a_sev, a_score)
